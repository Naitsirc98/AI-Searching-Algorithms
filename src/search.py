...
class GraphSearchResult:
    ...
    def __init__(self, node, expand_count):
        self.node = node
        self.expand_count = expand_count

    def __repr__(self):
        return "Path = %s, Expanded Nodes: %i" % (
            " => ".join([str(x) for x in self.node.path()]),
            self.expand_count)
...
def graph_search(problem, fringe):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            return GraphSearchResult(node, expand_count)
        if node.state not in closed:
            closed[node.state] = True
            expand = node.expand(problem)
            expand_count += 1
            fringe.extend(expand)
    return None
...
def acotation_graph_search(problem):
    return graph_search(problem, PriorityQueue(problem, lambda problem, x: x.path_cost))

def heuristic_graph_search(problem):
    return graph_search(problem, PriorityQueue(problem, lambda problem, x: problem.h(x) + x.path_cost))
